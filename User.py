    ...
    def __init__(self, parent, controller):
        ...
        def check_password():
            if account_no_entry.get() == "":
                messagebox.showinfo("SRM AP ATM Machine", " Account number cannot be empty.")
                ...
                if(len(password_entry_box.get()) == 4):
                    if (log) is not None:
                        ...
                        messagebox.showerror("SRM AP ATM Machine", " You do not have account.")
                        account_no_entry.delete(0, END)
                        password_entry_box.delete(0, END)
                else:
                    messagebox.showerror("SRM AP ATM Machine", " Your Password Should Be 4 Character")
            else:
                messagebox.showerror("SRM AP ATM Machine", " Something went wrong. Please try again later.")
    ...
    def __init__(self, parent, controller):
        ...
        def withdraw(amount):
            ...
            if (current_balance <=0) or ((current_balance -amount) <=0):
                messagebox.showinfo("SRM AP ATM Machine",
                                    "You Do not Have No Money")
            else:
                current_balance -= amount
                controller.shared_data['Balance'].set(current_balance)
                messagebox.showinfo("SRM AP ATM Machine", f" ${amount} was withdrawn."
        ...
        def other_amount(_):
            ...
            elif int(cash.get()) > current_balance:
                messagebox.showinfo("SRM AP ATM Machine", "There is not enough balance.")
            else:
                current_balance -= int(cash.get())
                controller.shared_data['Balance'].set(current_balance)
                messagebox.showinfo("SRM AP ATM Machine", f" ${int(cash.get())} was withdrawn."
                                                     f"\n You are being redirected to the home page.")
                model.update_balance(current_balance, controller.shared_data['Account'].get())
                cash.set('')
                controller.show_frame('MenuPage')
...
class DepositPage(Config):
    def __init__(self, parent, controller):
        ...
        def deposit_cash():
            global current_balance
            if int(cash.get()) <= 0:
                messagebox.showerror("SRM AP ATM Machine","Invalid amount.")
                deposit_entry.delete(0, END)
            elif int(cash.get()) >= 10000:
                messagebox.showinfo("SRM AP ATM Machine"," You can not deposit this amount. "
                                                    "\n You can do this from the nearest branch.")
                deposit_entry.delete(0, END)
            else:
                current_balance += int(cash.get())
                controller.shared_data['Balance'].set(current_balance)
                messagebox.showinfo("SRM AP ATM Machine", f" ${int(cash.get())} deposited."
                                                     f"\n You are being redirected to the home page.")
                model.update_balance(current_balance, controller.shared_data['Account'].get())
                controller.show_frame('MenuPage')
                cash.set('')
